...
import { PMLogModal } from './PMLogModal'
...
export function FleetDashboard({ profile }: { profile: FleetProfile }) {
  ...
  const [pmLogTruck,           setPmLogTruck]           = useState<Truck | null>(null)
    ...
    <div style={{ minHeight: '100vh', background: 'var(--surface)' }}>
      <main style={{ maxWidth: 1400, margin: '0 auto', padding: '1.5rem 1.25rem 4rem' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', marginTop: '1.25rem' }}>
          <StatusTable status={STATUS.OOS}    trucks={oosTrucks}    totalCount={trucks.filter(t => t.current_status === STATUS.OOS).length}    profile={profile} onStatusChange={handleStatusChange} onViewHistory={t => setNotesDrawer(t)} onInspect={t => setInspectTruck(t)} onUpdateWaitingOn={handleUpdateWaitingOn} onLogPM={t => setPmLogTruck(t)} />
          <StatusTable status={STATUS.ISSUES} trucks={issuesTrucks} totalCount={trucks.filter(t => t.current_status === STATUS.ISSUES).length} profile={profile} onStatusChange={handleStatusChange} onViewHistory={t => setNotesDrawer(t)} onInspect={t => setInspectTruck(t)} onUpdateWaitingOn={handleUpdateWaitingOn} onLogPM={t => setPmLogTruck(t)} />
          <StatusTable status={STATUS.READY}  trucks={readyTrucks}  totalCount={trucks.filter(t => t.current_status === STATUS.READY).length}  profile={profile} onStatusChange={handleStatusChange} onViewHistory={t => setNotesDrawer(t)} onInspect={t => setInspectTruck(t)} onUpdateWaitingOn={handleUpdateWaitingOn} onLogPM={t => setPmLogTruck(t)} />
        </div>
      </main>
      ...
      {pmLogTruck && (
        <PMLogModal
          truck={pmLogTruck}
          profile={profile}
          onClose={() => setPmLogTruck(null)}
          onSaved={() => { setPmLogTruck(null); fetchTrucks() }}
        />
      )}
    </div>
  )
