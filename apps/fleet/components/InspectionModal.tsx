...
export function InspectionModal({ truck, onClose, onSaved }: Props) {
  ...
  const ratingBtn = (key: string, val: InspectionItem['rating'], current: string, label: string, color: string) => (
    <button
      type="button"
      onClick={() => setRating(key, val)}
      style={{
        padding: '3px 10px', fontSize: 11, fontWeight: 700, borderRadius: 4, cursor: 'pointer',
        border: `1px solid ${current === val ? color : 'var(--outline)'}`,
        background: current === val ? color + '22' : 'transparent',
        color: current === val ? color : 'var(--on-surface-muted)',
      }}
    >
      {label}
    </button>
    ...
    <>
      ...
      <div style={{
        ...
        <form onSubmit={handleSubmit}>
          ...
          {/* Instruction note */}
          <p style={{ fontSize: '0.75rem', color: 'var(--on-surface-muted)', marginBottom: '1rem', fontStyle: 'italic' }}>
          ...
          {SECTIONS.map(section => (
            <div key={section.title} style={{ marginBottom: '1rem' }}>
              <div style={{
                fontSize: '0.75rem', fontWeight: 800, color: 'var(--primary)', textTransform: 'uppercase',
              ...
              {section.items.map(({ key, label }) => {
                ...
                return (
                  <div key={key} style={{ marginBottom: '0.625rem', padding: '0.5rem 0.625rem', background: 'var(--surface)', borderRadius: '0.375rem' }}>
                    <div style={{ display: 'flex', alignItems: 'flex-start', gap: '0.75rem', flexWrap: 'wrap' }}>
                      <span style={{ fontSize: '0.8125rem', color: 'var(--on-surface)', flex: 1, minWidth: 200, paddingTop: 2 }}>{label}</span>
                      <div style={{ display: 'flex', gap: '0.375rem', flexShrink: 0 }}>
                        {ratingBtn(key, 'ok',  item.rating, 'OK',  'var(--status-ready)')}
                        {ratingBtn(key, 'na',  item.rating, 'N/A', 'var(--on-surface-muted)')}
                        {ratingBtn(key, 'bad', item.rating, 'Bad', 'var(--error)')}
                      </div>
                    </div>
                    {item.rating === 'bad' && (
                      <input
